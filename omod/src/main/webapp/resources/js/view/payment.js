	...
	function(Backbone, openhmis, i18n) {
		...
		openhmis.PaymentView = Backbone.View.extend({
			...
			commitForm: function() {
				...
				for (var i in attributeForm) {
					...
					if (meta && meta.required === true && !attributeForm[i].value) {
						errors[attributeForm[i].name] = (__(openhmis.strings['openhmis.cashier.required']));
						break;
					}
			...
			processPayment: function(event) {
				...
				if (confirm(i18n(__(openhmis.strings['openhmis.cashier.paymentConfirm']), this.model.get("instanceType"), this.model.get("amountFmt")))) {
					var self = this;
