...
<form novalidate ng-show="fullyLoaded" name="entityForm" class="entity-form" ng-class="{'submitted': submitted}"
	...
	<fieldset class="content createBill">
		...
		<fieldset class="nested" ng-show="STATUS === 'PENDING'">
			...
			<table class="line-item">
				...
				<tr ng-repeat="lineItem in lineItems">
					...
					<td>
						<input id="quantity-{{\$index}}" class="form-control input-sm right-justify" type="number"
						       ng-model="lineItem.itemQuantity"
						       ng-change="changeItemQuantity(lineItem)" ng-enter="changeItemQuantity(lineItem)"/>
					</td>
					<td>
						<select ng-model="lineItem.itemPrice" class="form-control input-sm right-justify"
						        style="width:150px"
		...
		<fieldset class="paymentMode"
		          ng-show="STATUS !== 'PAID' && STATUS !== 'ADJUSTED' || (totalChangeDue < 0 && STATUS !== 'PENDING')">
			...
			<ul class="table-layout">
				...
				<li>
					<select class="form-control" ng-model="paymentMode"
					        ng-options="paymentMode.name for paymentMode in paymentModes"
			...
			<ul class="table-layout">
				<li>
					<input class="btn gray-button" type="button"
					       value="${ui.message('openhmis.cashier.bill.processPayment')}"
					       ng-click="processPayment()"
					       ng-disabled="amountTendered === 0 || (uuid === undefined && amountTendered <= 0) || amountTendered === undefined || processing === true"/>
				</li>
			</ul>
		</fieldset>
		...
		<span class="actions" ng-show="STATUS === 'PENDING'">
			...
			<input type="button" ng-disabled="processing === true" class="confirm right"
			       value="${ui.message('openhmis.cashier.bill.postAndPrint')}" ng-click="postAndPrintBill()"/>
			<input type="button" ng-disabled="processing === true" class="confirm btn gray-button right"
			       value="${ui.message('openhmis.cashier.bill.saveBill')}" ng-click="saveBill()"/>
			<input type="button" ng-disabled="processing === true" class="confirm btn gray-button right"
			       value="${ui.message('openhmis.cashier.bill.postBill')}" ng-click="postBill()"/>
		</span>
		
		<span class="actions" ng-show="STATUS === 'POSTED' || STATUS === 'PAID'">
			<input type="button" class="cancel" value="${ui.message('general.cancel')}" ng-click="cancel()"/>
			<input type="button" ng-disabled="processing === true" class="confirm btn gray-button right"
			       value="${ui.message('openhmis.cashier.bill.printReceipt')}" ng-click="printBill()"/>
			<input type="button" ng-disabled="processing === true" class="confirm btn gray-button right"
			       value="${ui.message('openhmis.cashier.bill.adjustBill')}" ng-click="adjustBill()"/>
		</span>
